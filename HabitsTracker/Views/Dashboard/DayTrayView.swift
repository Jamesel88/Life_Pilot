...
struct DayTrayView: View {
    ...
    var body: some View {
        VStack(spacing: 10) {
            VStack(spacing: 0) {
                ...
                // Tray of three compartments
                HStack(spacing: 0) {
                    compartment(label: "Tasks today", completed: tasksCompleted,
                                total: tasksTotal, color: .accentTasks)
                    divider
                    compartment(label: "Habits", completed: habitsDone,
                                total: habitsTotal, color: .accentHabits)
                    divider
                    compartment(label: "All time", completed: allTimeCompleted,
            ...
            // Labels under their compartments
            HStack(spacing: 0) {
    ...
    private var accessibilitySummary: String {
        var summary = "Today's overview. \(tasksCompleted) of \(tasksTotal) tasks done, "
            + "\(habitsDone) of \(habitsTotal) habits done, "
            + "\(allTimeCompleted) of \(allTimeTotal) tasks all time."
        if isSealed {
            summary += " The day is sealed — everything is done."
        }
        return summary
    }
}
